...
arx_sig0 = conj(rx_sig0);
ac0 = conv(rx_sig0,crx_sig0);
arx_sig1 = conj(rx_sig1);
ac1 = conv(rx_sig1,arx_sig1);
arx_sig2 = conj(rx_sig2);
ac2 = conv(rx_sig2,arx_sig2);
figure(2);
subplot(3,1,1);plot(abs(ac0));
ylabel('Autocorrelation');
title('u=25, N_{rb}=6');
subplot(3,1,2);plot(abs(ac1));
ylabel('Autocorrelation');
title('u=25, N_{rb}=25');
subplot(3,1,3);plot(abs(ac2));
ylabel('Autocorrelation');
title('u=25, N_{rb}=100');
